...
void calculate_workload_score(char *file_path)
{
    struct stat st;
    ...
    while(read(fd, &r, sizeof(Report)))
    {
        ...
        {
            //inca un for pentru a vedea daca numele este sau nu deja acolo
            int already_there = 0;

             for(int int i = 0; i < inspector_count; i++)
            {
            ...
            for(int int i = 0; i < inspector_count; i++)
            {
                if(strcmp(r.name, inspectors[i].name) == 0)
                {
                    inspectors[inspector_count].score += r.severity_level;
                    break;
                }
            }
        }
